...
from theme import SANS, THEMES, Theme
...
def render_radar(theme: Theme, title: str, axes: list[tuple[str, int]]) -> str:
    ...
    for level in range(1, 5):
        ...
        rings.append(f'<polygon points="{pts}" fill="none" stroke="{theme.border}" stroke-width="1"/>')

    axes_lines = []
    texts = []
    for i, label in enumerate(labels):
        x, y = _axis_point(cx, cy, r, i, n)
        axes_lines.append(f'<line x1="{cx}" y1="{cy}" x2="{x:.1f}" y2="{y:.1f}" stroke="{theme.border}"/>')
        lx, ly = _axis_point(cx, cy, r + 30, i, n)
        anchor = "middle" if abs(lx - cx) < 40 else ("end" if lx < cx else "start")
        texts.append(
            f'<text x="{lx:.1f}" y="{ly:.1f}" text-anchor="{anchor}" '
            f'fill="{theme.muted}" font-family="{SANS}" font-size="12.5">{label}</text>'
        )

    vertex_points = [_axis_point(cx, cy, r * v / 100, i, n) for i, v in enumerate(values)]
    poly_pts = " ".join(f"{x:.1f},{y:.1f}" for x, y in vertex_points)
    dots = "".join(
        f'<circle cx="{x:.1f}" cy="{y:.1f}" r="3.5" fill="{theme.bg}" stroke="{theme.violet}" stroke-width="2"/>'
        for x, y in vertex_points
    )

    return f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 420" role="img" aria-label="{title} skill radar">
<rect width="420" height="420" rx="20" fill="{theme.bg}"/>
{''.join(rings)}
{''.join(axes_lines)}
<polygon points="{poly_pts}" fill="{theme.violet}" fill-opacity=".18" stroke="{theme.violet}" stroke-width="2.5"/>
{dots}
{''.join(texts)}
<text x="210" y="32" text-anchor="middle" fill="{theme.text}" font-family="{SANS}" font-size="17" font-weight="700">{title}</text>
</svg>
"""
...
def main() -> None:
    ...
    charts = [
        ("radar", "Development focus", SKILL_AXES),
        ("radar-langs", "Language focus", LANG_AXES),
    ]
